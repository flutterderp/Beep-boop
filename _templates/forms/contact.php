...
require_once(LIB_PATH . '/libraries/recaptcha/autoload.php');
...
$doc->addScript('https://www.google.com/recaptcha/api.js', 'text/javascript', $defer = true);
...
if((count($_POST) > 0) && ($jpost_form_id == 'contact_form'))
{
	...
	{
		$errors[] = '&ldquo;' . htmlspecialchars($jpost_email) . '&rdquo; is not a valid email address';
	}
	if(!empty($jpost_phone) && !preg_match("/^([\d\s\(\)\-\+]{1,18})$/", $jpost_phone))
	{
		$errors[] = '&ldquo;' . htmlspecialchars($jpost_phone) . '&rdquo; is not a valid phone number';
	}
	...
	else
	{
		...
		$body .= '<p><small>As best as we could tell, this request came from ' . $from_ip . '.</small></p>';
		...
		if(!$mailer->send())
		{
			$app->enqueueMessage('Mail Error: ' . $mailer->ErrorInfo, 'warning');
		}
		else
		{
			$app->enqueueMessage('Your message has been sent.', 'success');
			// $app->redirect(JUri::root() . 'thank-you', null, null, false);
		}
	}
}
...
{
	?>
	<form action="<?php echo JFilterOutput::ampReplace(JUri::getInstance()->toString()); ?>#system-message-container" method="post"
		...
		<div class="row small-up-1 medium-up-2">
			...
			<div class="column">
				<label class="required">
					Email
					<input tabindex="0" type="text" id="txt_email" name="txt_email" value="<?php echo $jpost_email; ?>" required>
				</label>
			</div>
			...
			<div class="column">
				<label>
					Phone
					<input tabindex="0" type="text" id="txt_phone" name="txt_phone" value="<?php echo $jpost_phone; ?>">
				</label>
			</div>
