...
function modChrome_spanTitle($module, &$params, &$attribs)
{
	...
	if($module->content)
	{
		echo '<' . $moduleTag . ' class="moduletable' . $moduleClass . '" id="' . $moduleId . JFilterOutput::stringURLSafe($module->title) . '" aria-label="' . $moduleId . $module->title . '">';
...
function modChrome_padCustom($module, &$params, &$attribs)
{
	...
	if($module->content)
	{
		...
		{
			echo '<img src="' . $moduleImg . '" alt="' . JFilterOutput::stringURLSafe($module->title) . '">';
		}
...
function modChrome_xhtml5($module, &$params, &$attribs)
{
	...
	if(!empty ($module->content))
	{
		echo '<' . $moduleTag . ' class="moduletable' . htmlspecialchars($params->get('moduleclass_sfx'), ENT_COMPAT, 'UTF-8') . $moduleClass .
			'" id="' . $moduleId . JFilterOutput::stringURLSafe($module->title) . '" aria-label="' . $moduleId . $module->title . '">';
